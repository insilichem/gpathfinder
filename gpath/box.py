...
"""
This module is a messy collection of useful functions used all along GPATH.
...
def create_single_individual(path):
    ...
    def prepare_input(path, parser):
        ...
        for g in cfg.genes:
            if g.module == 'gpath.genes.molecule':
...
def find_nearest(anchor, atoms):
    ...
    structures, using ``gpath.box.shortest_distance_matrix`` as
    done in ``gpath.genes.torsion.Torsion._compute_rotatable_bonds``
    is recommended.
    """
    try:
