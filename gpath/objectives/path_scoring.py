...
class PathScoring(ObjectiveProvider):
    ...
    _validate = {
        ...
        'smoothness_molecules': [parse.In(['Ligand', 'Protein'])],
    ...
    def __init__(self, probe, ligand='Ligand', protein='Protein', radius=5.0,
                 ...
                 smoothness_threshold=0.0, smoothness_molecules=['Ligand'],
                 metal_atoms=None,
                 metal_residues=['ARG', 'ASN', 'ASP', 'CYS', 'GLN', 'GLU', 'HIS', 'MET', 'SER', 'THR', 'TYR'],
                 metal_backbone=False, smina_scoring=None,
                 smina_custom_scoring=None,
                 smina_custom_atoms=None, *args, **kwargs):
        ...
        if which == 'clashes':
            ...
            self.smoothness_molecules = smoothness_molecules
    ...
    def evaluate_smoothness(self, ind):
        ...
        #Frame 0
        ind.genes[self._probe].gp_express(0, with_rotamers=False, smoothness_molecules=self.smoothness_molecules)
        if "Ligand" in self.smoothness_molecules:
        ...
        for i in range(1, len(ind.genes[self._probe].allele['positions'])):
            ...
            if not 'smoothness' in scores[i].keys():
                ind.genes[self._probe].gp_express(i, with_rotamers=False, smoothness_molecules=self.smoothness_molecules)

                #Calculate smoothness
                smooth_point = []
                if "Ligand" in self.smoothness_molecules:
                    current_ligand_atoms_coords = self._get_mol_coords(self.ligand(ind))
                    smooth_point.append(self._distance_atom_sets(prev_ligand_atoms_coords, current_ligand_atoms_coords))
                if "Protein" in self.smoothness_molecules:
